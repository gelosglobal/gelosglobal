...
import { computeSfProductPerformance } from '@/lib/sf-product-performance'
...
export async function GET(request: Request) {
  ...
  if (preset === 'custom') {
    ...
    if (!s || !e || e.getTime() < s.getTime()) {
      return NextResponse.json({ error: 'Invalid date range' }, { status: 400, headers: noStore })
    }
    currentStart = s
  ...
  const { db } = getMongo()
  const { rows, highlights } = await computeSfProductPerformance(db, { currentStart, currentEnd })
