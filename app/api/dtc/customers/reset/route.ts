...
/**
 * Removes every document in `dtc_customers` (explicit customer rows only).
 * `dtc_orders` and other collections are unchanged.
 */
export async function POST(request: Request) {
  ...
  const { db } = getMongo()
  const result = await db.collection(DTC_CUSTOMERS_COLLECTION).deleteMany({})
